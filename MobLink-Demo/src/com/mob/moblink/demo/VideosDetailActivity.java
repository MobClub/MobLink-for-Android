...
public class VideosDetailActivity extends ShareableActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent() != null) {
			videoID = getIntent().getIntExtra("position", videoID);
		}
		setVideoDetail();
	}
	...
	private void getMobIDToShare() {
		...
		Scene s = new Scene();
		s.setPath(CommonUtils.VIDEO_PATH);
		s.setParams(params);
	...
	private void share() {
		String shareUrl = CommonUtils.getShareUrl() + CommonUtils.VIDEO_PATH + "/" + videoID;
		if (!TextUtils.isEmpty(mobID)) {
	...
	public void onReturnSceneData(Scene scene) {
		if (scene != null) {
			if (null != scene.getParams()) {
				String value = null;
				if (scene.getParams().containsKey("videoID")) {
					value = String.valueOf(scene.getParams().get("videoID"));
