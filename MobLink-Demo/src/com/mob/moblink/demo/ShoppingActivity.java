...
public class ShoppingActivity extends ShareableActivity {
	...
	public void onReturnSceneData(Scene scene) {
		...
		final int goodsId = null != scene && null != scene.getParams() ? (Integer) scene.getParams().get("goodsID") : null;
		dialog = CommonUtils.getRestoreSceneDialog(ShoppingActivity.this, new Runnable() {
			public void run() {
				Intent intent = new Intent(ShoppingActivity.this, ShoppingDetailActivity.class);
				intent.putExtra("position", goodsId);
				startActivity(intent);
			}
		});
		dialog.show();
