...
public class ActivityA extends Activity implements View.OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		path = CommonUtils.MAIN_PATH_ARR[0];
	}
	...
	protected void onResume() {
		...
		MobLink.setIntentHandler(getIntent(), new ActionListener() {
			public void onResult(final HashMap<String, Object> res) {
				runOnUiThread(new Runnable() {
					public void run() {
						...
						if (dialog == null) {
							dialog = CommonUtils.getDialog(ActivityA.this, path, source, paramStr);
						}
						if (!dialog.isShowing()) {
							dialog.show();
						}
			...
			public void onError(Throwable t) {
				if (t != null) {
					Toast.makeText(ActivityA.this, t.getMessage(), Toast.LENGTH_SHORT).show();
				}
			}
		});
